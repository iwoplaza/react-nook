...
import { nook, nookEffect, nookState } from 'react-nook';
...
const modCounter = nook((mod: number) => {
  const [value, setValue] = nookState``(0);
...
const toggle = nook((initial: boolean) => {
  const [value, setValue] = nookState``(initial);
...
const interval = nook((cb: () => unknown, interval: number) => {
  nookEffect``(() => {
    const handle = window.setInterval(cb, interval);
